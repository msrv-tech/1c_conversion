# -*- coding: utf-8 -*-
"""
Модуль выгрузки статьидвиженияденежныхсредств из обработанной БД в 1С приемник.
...
def write_cash_flow_items_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает статьидвиженияденежныхсредств из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА СТАТЬИДВИЖЕНИЯДЕНЕЖНЫХСРЕДСТВ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение статьидвиженияденежныхсредств из обработанной БД...")
    items = get_from_db(db_connection, "cash_flow_items")
    db_connection.close()
    
    if not items:
        verbose_print("СтатьиДвиженияДенежныхСредств не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} статьидвиженияденежныхсредств...")
    written_count = 0
    error_count = 0
