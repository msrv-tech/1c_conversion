# -*- coding: utf-8 -*-
"""
Модуль выгрузки бланкистрогойотчетности из обработанной БД в 1С приемник.
...
def write_strict_reporting_forms_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает бланкистрогойотчетности из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА БЛАНКИСТРОГОЙОТЧЕТНОСТИ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение бланкистрогойотчетности из обработанной БД...")
    items = get_from_db(db_connection, "strict_reporting_forms")
    db_connection.close()
    
    if not items:
        verbose_print("БланкиСтрогойОтчетности не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} бланкистрогойотчетности...")
    written_count = 0
    error_count = 0
