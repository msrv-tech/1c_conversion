# -*- coding: utf-8 -*-
"""
Модуль выгрузки статьирасходов из обработанной БД в 1С приемник.
...
def write_cost_items_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает статьирасходов из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА СТАТЬИРАСХОДОВ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение статьирасходов из обработанной БД...")
    items = get_from_db(db_connection, "cost_items")
    db_connection.close()
    
    if not items:
        verbose_print("СтатьиРасходов не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} статьирасходов...")
    written_count = 0
    error_count = 0
