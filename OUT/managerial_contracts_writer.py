# -*- coding: utf-8 -*-
"""
Модуль выгрузки управленческихдоговоров из обработанной БД в 1С приемник.
...
def _write_item(com_object, item_data, processed_db=None):
    ...
    """
    # Если наименование пустое, заполняем его кодом
    ...
    if not name_value or (isinstance(name_value, str) and not name_value.strip()):
        ...
        if code_value:
            item_data['Наименование'] = str(code_value)
            verbose_print(f"  → Наименование пустое, заполнено кодом: {code_value}")
    
    return write_catalog_item(
        com_object,
        item_data,
        "custom_УправленческиеДоговоры",
        "Справочник.custom_УправленческиеДоговоры",
        ["Код", "Наименование", "ПометкаУдаления", "Комментарий"],
        processed_db=processed_db
    )


def write_managerial_contracts_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает управленческиедоговоры из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА УПРАВЛЕНЧЕСКИХДОГОВОРОВ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение управленческихдоговоров из обработанной БД...")
    items = get_from_db(db_connection, "managerial_contracts")
    db_connection.close()
    
    if not items:
        verbose_print("УправленческиеДоговоры не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} управленческихдоговоров...")
    written_count = 0
    error_count = 0
