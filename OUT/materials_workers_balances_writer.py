# -*- coding: utf-8 -*-
"""
Модуль выгрузки остатков по счёту МЦЭ (МЦ на работниках) в документ «Ввод начальных остатков»,
...
COMMENT_MARKER = "### Загрузка остатков МЦ на работниках (31.12.2025) ###"

# В УПП один забалансовый счёт МЦЭ; целевой счёт в приёмнике задаётся по виду номенклатуры
# (Спецодежда и спецоснастка на складе 10.10 -> МЦ.02, Хоз. инвентарь 10.09 -> МЦ.04, иначе МЦ.03).
# TargetAccountCode заполняется в загрузчике.
...
def _get_target_account_code_from_item(item: dict) -> str:
    """Код счёта в приёмнике: из поля TargetAccountCode (МЦ.02, МЦ.03, МЦ.04), задаётся по виду номенклатуры в загрузчике."""
    code = (item.get("TargetAccountCode") or "").strip()
    if code in ("МЦ.02", "МЦ.03", "МЦ.04"):
        return code
    return "МЦ.03"


def write_materials_workers_balances_to_1c(sqlite_db_file: str, com_object: object, process_func=None) -> bool:
    """Записывает остатки МЦЭ в документ Ввод начальных остатков, ТЧ БухСправка, раздел ПрочиеСчетаБухгалтерскогоУчета."""
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА ОСТАТКОВ МЦ (НА РАБОТНИКАХ) В ДОКУМЕНТ ВВОД ОСТАТКОВ")
    verbose_print("=" * 80)
    ...
    items = get_from_db(db_connection, "materials_workers_balances")
    db_connection.close()
    ...
    try:
        ...
        for org_uuid, group_items in doc_groups.items():
            ...
            for item in group_items:
                ...
                # СчетУчета: только МЦ.02, МЦ.03 или МЦ.04 по виду номенклатуры (TargetAccountCode из загрузчика)
                target_code = _get_target_account_code_from_item(item)
                ...
                # Субконто1 приёмника = Номенклатура (источник Субконто2)
                ...
                # Субконто2 приёмника = Партии материалов в эксплуатации (источник Характеристика — по UUID при возможности)
                if hasattr(new_row, "Субконто2"):
                    ref2 = _ref_by_uuid_or_type(
                        com_object,
                        item.get("Характеристика"),
                        "Документ.ПартияМатериаловВЭксплуатации",
                        processed_db=sqlite_db_file,
                    )
                    if ref2:
                        new_row.Субконто2 = ref2

                # Субконто3 приёмника = Работники организаций (источник Субконто1)
                if hasattr(new_row, "Субконто3"):
                    ref3 = _ref_by_uuid_or_type(
                        com_object,
                        item.get("РаботникиОрганизации"),
...
if __name__ == "__main__":
    ...
    if com:
        write_materials_workers_balances_to_1c("BD/materials_workers_balances_processed.db", com)
