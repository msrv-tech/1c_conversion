# -*- coding: utf-8 -*-
"""
Модуль выгрузки причинысписанияос из обработанной БД в 1С приемник.
...
def write_fixed_asset_writeoff_reasons_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает причинысписанияос из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА ПРИЧИНЫСПИСАНИЯОС ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение причинысписанияос из обработанной БД...")
    items = get_from_db(db_connection, "fixed_asset_writeoff_reasons")
    db_connection.close()
    
    if not items:
        verbose_print("ПричиныСписанияОС не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} причинысписанияос...")
    written_count = 0
    error_count = 0
