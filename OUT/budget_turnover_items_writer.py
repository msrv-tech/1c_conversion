# -*- coding: utf-8 -*-
"""
Модуль выгрузки статьиоборотовпобюджетам из обработанной БД в 1С приемник.
...
def write_budget_turnover_items_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает статьиоборотовпобюджетам из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА СТАТЬИОБОРОТОВПОБЮДЖЕТАМ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение статьиоборотовпобюджетам из обработанной БД...")
    items = get_from_db(db_connection, "budget_turnover_items")
    db_connection.close()
    
    if not items:
        verbose_print("СтатьиОборотовПоБюджетам не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} статьиоборотовпобюджетам...")
    written_count = 0
    error_count = 0
