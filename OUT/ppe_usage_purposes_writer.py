# -*- coding: utf-8 -*-
"""
Модуль выгрузки назначенияиспользованияноменклатуры из обработанной БД в 1С приемник.
...
def _write_item(com_object, item_data, processed_db=None):
    ...
    return write_catalog_item(
        ...
        "Справочник.НазначенияИспользованияНоменклатуры",
        ["Код", "Наименование", "ПометкаУдаления", "Комментарий"],
        processed_db=processed_db
    )


def write_ppe_usage_purposes_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает назначенияиспользованияноменклатуры из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА НАЗНАЧЕНИЯИСПОЛЬЗОВАНИЯНОМЕНКЛАТУРЫ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение назначенияиспользованияноменклатуры из обработанной БД...")
    items = get_from_db(db_connection, "ppe_usage_purposes")
    db_connection.close()
    
    if not items:
        verbose_print("НазначенияИспользованияНоменклатуры не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} назначенияиспользованияноменклатуры...")
    written_count = 0
    error_count = 0
