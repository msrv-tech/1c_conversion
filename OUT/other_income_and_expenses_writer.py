# -*- coding: utf-8 -*-
"""
Модуль выгрузки прочиедоходыирасходы из обработанной БД в 1С приемник.
...
def write_other_income_and_expenses_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает прочиедоходыирасходы из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА ПРОЧИЕДОХОДЫИРАСХОДЫ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение прочиедоходыирасходы из обработанной БД...")
    items = get_from_db(db_connection, "other_income_and_expenses")
    db_connection.close()
    
    if not items:
        verbose_print("ПрочиеДоходыИРасходы не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} прочиедоходыирасходы...")
    written_count = 0
    error_count = 0
