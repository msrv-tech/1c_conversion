# -*- coding: utf-8 -*-
"""
Модуль выгрузки номенклатуры из обработанной БД в 1С приемник.
...
def _write_nomenclature_item(com_object, nomenclature_data, processed_db=None):
    ...
    return write_catalog_item(
        ...
        ['Код', 'Наименование', 'ПометкаУдаления', 'Артикул', 'Комментарий'],  # ВидНоменклатуры - ссылочное поле, не стандартное
        processed_db=processed_db
    )


def write_nomenclature_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает номенклатуру из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА НОМЕНКЛАТУРЫ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    # Шаг 2: Чтение номенклатуры из БД
    verbose_print("\n[2/3] Чтение номенклатуры из обработанной БД...")
    nomenclatures = get_from_db(db_connection, "nomenclature")
    db_connection.close()
    
    if not nomenclatures:
        verbose_print("Номенклатура не найдена в базе данных")
        return False
    ...
    verbose_print(f"\nНачинаем запись {len(nomenclatures)} номенклатуры...")
    written_count = 0
    error_count = 0
