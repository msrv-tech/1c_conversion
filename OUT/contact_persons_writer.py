# -*- coding: utf-8 -*-
"""
Модуль выгрузки контактныелица из обработанной БД в 1С приемник.
...
def write_contact_persons_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает контактныелица из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА КОНТАКТНЫЕЛИЦА ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение контактныелица из обработанной БД...")
    items = get_from_db(db_connection, "contact_persons")
    db_connection.close()
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} контактныелица...")
    written_count = 0
    error_count = 0
