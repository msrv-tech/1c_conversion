# -*- coding: utf-8 -*-
"""
Модуль выгрузки способыотражениярасходовпоамортизации из обработанной БД в 1С приемник.
...
def _write_item(com_object, item_data, processed_db=None):
    ...
    return write_catalog_item(
        ...
        "Справочник.СпособыОтраженияРасходовПоАмортизации",
        ["Код", "Наименование", "ПометкаУдаления", "Комментарий"],
        processed_db=processed_db
    )


def write_amortization_expense_methods_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает способыотражениярасходовпоамортизации из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА СПОСОБЫОТРАЖЕНИЯРАСХОДОВПОАМОРТИЗАЦИИ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение способыотражениярасходовпоамортизации из обработанной БД...")
    items = get_from_db(db_connection, "amortization_expense_methods")
    db_connection.close()
    
    if not items:
        verbose_print("СпособыОтраженияРасходовПоАмортизации не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} способыотражениярасходовпоамортизации...")
    written_count = 0
    error_count = 0
