# -*- coding: utf-8 -*-
"""
Модуль выгрузки нематериальныеактивы из обработанной БД в 1С приемник.
...
def write_intangible_assets_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает нематериальныеактивы из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА НЕМАТЕРИАЛЬНЫЕАКТИВЫ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение нематериальныеактивы из обработанной БД...")
    items = get_from_db(db_connection, "intangible_assets")
    db_connection.close()
    
    if not items:
        verbose_print("НематериальныеАктивы не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} нематериальныеактивы...")
    written_count = 0
    error_count = 0
