# -*- coding: utf-8 -*-
"""
Модуль выгрузки расходыбудущихпериодов из обработанной БД в 1С приемник.
...
def write_prepaid_expenses_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает расходыбудущихпериодов из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА РАСХОДЫБУДУЩИХПЕРИОДОВ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение расходыбудущихпериодов из обработанной БД...")
    items = get_from_db(db_connection, "prepaid_expenses")
    db_connection.close()
    
    if not items:
        verbose_print("РасходыБудущихПериодов не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} расходыбудущихпериодов...")
    written_count = 0
    error_count = 0
