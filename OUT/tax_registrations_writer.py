# -*- coding: utf-8 -*-
"""
Модуль выгрузки регистрациявифнс из обработанной БД в 1С приемник.
...
def write_tax_registrations_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает регистрациявифнс из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА РЕГИСТРАЦИЯВИФНС ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение регистрациявифнс из обработанной БД...")
    items = get_from_db(db_connection, "tax_registrations")
    db_connection.close()
    
    if not items:
        verbose_print("РегистрацияВИФНС не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} регистрациявифнс...")
    written_count = 0
    error_count = 0
