# -*- coding: utf-8 -*-
"""
Модуль выгрузки субконто из обработанной БД в 1С приемник.
...
def write_subkonto_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает субконто из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА СУБКОНТО ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение субконто из обработанной БД...")
    items = get_from_db(db_connection, "subkonto")
    db_connection.close()
    
    if not items:
        verbose_print("Субконто не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} субконто...")
    written_count = 0
    error_count = 0
