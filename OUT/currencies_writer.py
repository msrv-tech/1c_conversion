# -*- coding: utf-8 -*-
"""
Модуль выгрузки валюты из обработанной БД в 1С приемник.
...
def _write_item(com_object, item_data, processed_db=None):
    ...
    return write_catalog_item(
        ...
        "Валюты",
        "Справочник.Валюты",
        ["Код", "Наименование", "ПометкаУдаления", "НаименованиеПолное", "ПараметрыПрописиНаРусском"],
        processed_db=processed_db
    )


def write_currencies_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает валюты из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА ВАЛЮТЫ ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение валюты из обработанной БД...")
    items = get_from_db(db_connection, "currencies")
    db_connection.close()
    
    if not items:
        verbose_print("Валюты не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} валюты...")
    written_count = 0
    error_count = 0
