# -*- coding: utf-8 -*-
"""
Модуль выгрузки объектыстроительства из обработанной БД в 1С приемник.
...
def write_construction_objects_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает объектыстроительства из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА ОБЪЕКТЫСТРОИТЕЛЬСТВА ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    verbose_print("\n[2/3] Чтение объектыстроительства из обработанной БД...")
    items = get_from_db(db_connection, "construction_objects")
    db_connection.close()
    
    if not items:
        verbose_print("ОбъектыСтроительства не найдены в базе данных")
        return False
    ...
    # Записываем элементы
    verbose_print(f"\nНачинаем запись {len(items)} объектыстроительства...")
    written_count = 0
    error_count = 0
