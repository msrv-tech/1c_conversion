# -*- coding: utf-8 -*-
"""
Модуль выгрузки документов «Заказ покупателя» (преобразованных в справочник «ДоговорыКонтрагентов»)
...
def _write_item(com_object, item_data, processed_db=None):
    ...
    success = write_catalog_item(
        ...
        source_object_name="ЗаказПокупателя",
        source_object_type="document"
    )
    
    # Догрузка: обновляем НоменклатурныеГруппы.customДоговор = записанный договор
    # Если НоменклатурнаяГруппа не найдена — создаём неполную запись (filled=False) для дальнейшей догрузки
...
def write_customer_orders_to_1c(sqlite_db_file, com_object, process_func=None):
    """
    Выгружает документы ЗаказПокупателя (преобразованные в ДоговорыКонтрагентов) 
    из обработанной БД в 1С приемник.
    ...
    """
    verbose_print("=" * 80)
    verbose_print("ВЫГРУЗКА ДОКУМЕНТОВ ЗАКАЗПОКУПАТЕЛЯ (→ ДОГОВОРЫКОНТРАГЕНТОВ) ИЗ ОБРАБОТАННОЙ БД В 1С ПРИЕМНИК")
    verbose_print("=" * 80)
    ...
    items = get_from_db(db_connection, "customer_orders_processed")
    db_connection.close()
