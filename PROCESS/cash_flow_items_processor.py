# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «СтатьиДвиженияДенежныхСредств» с использованием маппинга типов и полей.

Читает статьидвиженияденежныхсредств из исходной БД, применяет маппинг из type_mapping.db
...
class CashFlowItemsMappingProcessor(MappingProcessor):
    """Процессор для преобразования статьидвиженияденежныхсредств с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "СтатьиДвиженияДенежныхСредств", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} статьидвиженияденежныхсредств...")
        processed = []
        ...
        verbose_print(f"Обработано статьидвиженияденежныхсредств: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "cash_flow_items"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "cash_flow_items")
        ...
        if not processed:
            verbose_print("Нет обработанных статьидвиженияденежныхсредств для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено статьидвиженияденежныхсредств в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении статьидвиженияденежныхсредств: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_cash_flow_items(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает статьидвиженияденежныхсредств из исходной БД и сохраняет в новую БД.
    ...
    table_name = "cash_flow_items"
    
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА СТАТЬИДВИЖЕНИЯДЕНЕЖНЫХСРЕДСТВ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение статьидвиженияденежныхсредств из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("СтатьиДвиженияДенежныхСредств не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано статьидвиженияденежныхсредств: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = CashFlowItemsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка статьидвиженияденежныхсредств с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка статьидвиженияденежныхсредств с маппингом")
    ...
    success = process_cash_flow_items(args.source_db, args.processed_db)
