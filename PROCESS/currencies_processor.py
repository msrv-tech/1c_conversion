# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «Валюты» с использованием маппинга типов и полей.

Читает валюты из исходной БД, применяет маппинг из type_mapping.db
...
class CurrenciesMappingProcessor(MappingProcessor):
    """Процессор для преобразования валюты с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "Валюты", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        verbose_print(f"Обработано валюты: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "currencies"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "currencies")
        ...
        if not processed:
            verbose_print("Нет обработанных валюты для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено валюты в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении валюты: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_currencies(
    ...
    """
    Обрабатывает валюты из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ВАЛЮТЫ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение валюты из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("Валюты не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано валюты: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = CurrenciesMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка валюты с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка валюты с маппингом")
    ...
    success = process_currencies(args.source_db, args.processed_db)
