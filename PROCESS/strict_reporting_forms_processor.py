# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «БланкиСтрогойОтчетности» с использованием маппинга типов и полей.

Читает бланкистрогойотчетности из исходной БД, применяет маппинг из type_mapping.db
...
class StrictReportingFormsMappingProcessor(MappingProcessor):
    """Процессор для преобразования бланкистрогойотчетности с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "БланкиСтрогойОтчетности", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} бланкистрогойотчетности...")
        processed = []
        ...
        verbose_print(f"Обработано бланкистрогойотчетности: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "strict_reporting_forms"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "strict_reporting_forms")
        ...
        if not processed:
            verbose_print("Нет обработанных бланкистрогойотчетности для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено бланкистрогойотчетности в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении бланкистрогойотчетности: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_strict_reporting_forms(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает бланкистрогойотчетности из исходной БД и сохраняет в новую БД.
    ...
    table_name = "strict_reporting_forms"
    
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА БЛАНКИСТРОГОЙОТЧЕТНОСТИ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение бланкистрогойотчетности из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("БланкиСтрогойОтчетности не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано бланкистрогойотчетности: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = StrictReportingFormsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка бланкистрогойотчетности с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка бланкистрогойотчетности с маппингом")
    ...
    success = process_strict_reporting_forms(args.source_db, args.processed_db)
