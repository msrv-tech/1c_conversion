# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «РегистрацияВИФНС» с использованием маппинга типов и полей.

Читает регистрациявифнс из исходной БД, применяет маппинг из type_mapping.db
...
class TaxRegistrationsMappingProcessor(MappingProcessor):
    """Процессор для преобразования регистрациявифнс с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "РегистрацияВИФНС", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} регистрациявифнс...")
        processed = []
        ...
        verbose_print(f"Обработано регистрациявифнс: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "tax_registrations"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "tax_registrations")
        ...
        if not processed:
            verbose_print("Нет обработанных регистрациявифнс для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено регистрациявифнс в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении регистрациявифнс: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_tax_registrations(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает регистрациявифнс из исходной БД и сохраняет в новую БД.
    ...
    table_name = "tax_registrations"
    
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА РЕГИСТРАЦИЯВИФНС С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение регистрациявифнс из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("РегистрацияВИФНС не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано регистрациявифнс: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = TaxRegistrationsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка регистрациявифнс с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка регистрациявифнс с маппингом")
    ...
    success = process_tax_registrations(args.source_db, args.processed_db)
