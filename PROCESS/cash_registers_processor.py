# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «Кассы» с использованием маппинга типов и полей.

Читает кассы из исходной БД, применяет маппинг из type_mapping.db
...
class CashRegistersMappingProcessor(MappingProcessor):
    """Процессор для преобразования кассы с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "Кассы", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        verbose_print(f"Обработано кассы: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "cash_registers"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "cash_registers")
        ...
        if not processed:
            verbose_print("Нет обработанных кассы для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено кассы в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении кассы: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_cash_registers(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает кассы из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА КАССЫ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение кассы из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("Кассы не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано кассы: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = CashRegistersMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка кассы с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка кассы с маппингом")
    ...
    success = process_cash_registers(args.source_db, args.processed_db)
