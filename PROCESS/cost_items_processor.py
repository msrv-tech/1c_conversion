# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «СтатьиРасходов» с использованием маппинга типов и полей.

Читает статьирасходов из исходной БД, применяет маппинг из type_mapping.db
...
class CostItemsMappingProcessor(MappingProcessor):
    """Процессор для преобразования статьирасходов с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "СтатьиЗатрат", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} статьирасходов...")
        processed = []
        ...
        verbose_print(f"Обработано статьирасходов: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "cost_items"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "cost_items")
        ...
        if not processed:
            verbose_print("Нет обработанных статьирасходов для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено статьирасходов в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении статьирасходов: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_cost_items(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает статьирасходов из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА СТАТЬИРАСХОДОВ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение статьирасходов из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("СтатьиРасходов не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано статьирасходов: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = CostItemsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка статьирасходов с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка статьирасходов с маппингом")
    ...
    success = process_cost_items(args.source_db, args.processed_db)
