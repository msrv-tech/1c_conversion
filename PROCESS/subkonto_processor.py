# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «Субконто» с использованием маппинга типов и полей.

Читает субконто из исходной БД, применяет маппинг из type_mapping.db
...
class SubkontoMappingProcessor(MappingProcessor):
    """Процессор для преобразования субконто с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "Субконто", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        verbose_print(f"Обработано субконто: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "subkonto"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "subkonto")
        ...
        if not processed:
            verbose_print("Нет обработанных субконто для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено субконто в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении субконто: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_subkonto(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает субконто из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА СУБКОНТО С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение субконто из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("Субконто не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано субконто: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = SubkontoMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка субконто с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка субконто с маппингом")
    ...
    success = process_subkonto(args.source_db, args.processed_db)
