# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «ОбъектыСтроительства» с использованием маппинга типов и полей.

Читает объектыстроительства из исходной БД, применяет маппинг из type_mapping.db
...
class ConstructionObjectsMappingProcessor(MappingProcessor):
    """Процессор для преобразования объектыстроительства с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "ОбъектыСтроительства", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} объектыстроительства...")
        processed = []
        ...
        verbose_print(f"Обработано объектыстроительства: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "construction_objects"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "construction_objects")
        ...
        if not processed:
            verbose_print("Нет обработанных объектыстроительства для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено объектыстроительства в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении объектыстроительства: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_construction_objects(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает объектыстроительства из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ОБЪЕКТЫСТРОИТЕЛЬСТВА С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение объектыстроительства из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("ОбъектыСтроительства не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано объектыстроительства: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = ConstructionObjectsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка объектыстроительства с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка объектыстроительства с маппингом")
    ...
    success = process_construction_objects(args.source_db, args.processed_db)
