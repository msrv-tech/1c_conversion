# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «Банки» с использованием маппинга типов и полей.

Читает банки из исходной БД, применяет маппинг из type_mapping.db
...
class BanksMappingProcessor(MappingProcessor):
    """Процессор для преобразования банки с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "Банки", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        verbose_print(f"Обработано банки: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "banks"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "banks")
        ...
        if not processed:
            verbose_print("Нет обработанных банки для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено банки в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении банки: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_banks(
    ...
    """
    Обрабатывает банки из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА БАНКИ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение банки из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("Банки не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано банки: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = BanksMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка банки с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка банки с маппингом")
    ...
    success = process_banks(args.source_db, args.processed_db)
