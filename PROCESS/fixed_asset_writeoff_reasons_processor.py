# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «ПричиныСписанияОС» с использованием маппинга типов и полей.

Читает причинысписанияос из исходной БД, применяет маппинг из type_mapping.db
...
class FixedAssetWriteoffReasonsMappingProcessor(MappingProcessor):
    """Процессор для преобразования причинысписанияос с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "ПричиныСписанияОС", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} причинысписанияос...")
        processed = []
        ...
        verbose_print(f"Обработано причинысписанияос: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "fixed_asset_writeoff_reasons"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "fixed_asset_writeoff_reasons")
        ...
        if not processed:
            verbose_print("Нет обработанных причинысписанияос для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено причинысписанияос в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении причинысписанияос: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_fixed_asset_writeoff_reasons(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает причинысписанияос из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ПРИЧИНЫСПИСАНИЯОС С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение причинысписанияос из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("ПричиныСписанияОС не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано причинысписанияос: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = FixedAssetWriteoffReasonsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка причинысписанияос с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка причинысписанияос с маппингом")
    ...
    success = process_fixed_asset_writeoff_reasons(args.source_db, args.processed_db)
