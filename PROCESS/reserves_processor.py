# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «Резервы» с использованием маппинга типов и полей.

Читает резервы из исходной БД, применяет маппинг из type_mapping.db
...
class ReservesMappingProcessor(MappingProcessor):
    """Процессор для преобразования резервы с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "Резервы", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        verbose_print(f"Обработано резервы: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "reserves"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "reserves")
        ...
        if not processed:
            verbose_print("Нет обработанных резервы для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено резервы в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении резервы: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_reserves(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает резервы из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА РЕЗЕРВЫ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение резервы из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("Резервы не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано резервы: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = ReservesMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка резервы с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка резервы с маппингом")
    ...
    success = process_reserves(args.source_db, args.processed_db)
