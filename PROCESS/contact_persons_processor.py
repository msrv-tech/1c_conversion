# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «КонтактныеЛица» с использованием маппинга типов и полей.

Читает контактныелица из исходной БД, применяет маппинг из type_mapping.db
...
class ContactPersonsMappingProcessor(MappingProcessor):
    """Процессор для преобразования контактныелица с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "КонтактныеЛица", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} контактныелица...")
        processed = []
        ...
        verbose_print(f"Обработано контактныелица: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "contact_persons"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "contact_persons")
        ...
        if not processed:
            verbose_print("Нет обработанных контактныелица для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено контактныелица в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении контактныелица: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_contact_persons(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает контактныелица из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА КОНТАКТНЫЕЛИЦА С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение контактныелица из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("КонтактныеЛица не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано контактныелица: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = ContactPersonsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка контактныелица с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка контактныелица с маппингом")
    ...
    success = process_contact_persons(args.source_db, args.processed_db)
