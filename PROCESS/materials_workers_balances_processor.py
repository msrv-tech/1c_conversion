# -*- coding: utf-8 -*-
"""
Модуль обработки остатков по счёту МЦЭ (МЦ на работниках).
...
SOURCE_TABLE = "materials_workers_balances"
PROCESSED_TABLE = "materials_workers_balances"


def process_materials_workers_balances(source_db_path: str, processed_db_path: str) -> bool:
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ОСТАТКОВ ПО СЧЁТУ МЦЭ (МЦ НА РАБОТНИКАХ)")
    verbose_print("=" * 80)
...
if __name__ == "__main__":
    process_materials_workers_balances(
        "BD/materials_workers_balances.db",
        "BD/materials_workers_balances_processed.db",
    )
