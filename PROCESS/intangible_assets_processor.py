# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «НематериальныеАктивы» с использованием маппинга типов и полей.

Читает нематериальныеактивы из исходной БД, применяет маппинг из type_mapping.db
...
class IntangibleAssetsMappingProcessor(MappingProcessor):
    """Процессор для преобразования нематериальныеактивы с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "НематериальныеАктивы", "catalog")
    ...
    def process_item_single(self, item: Dict) -> Dict:
        ...
        # Обрабатываем поле ВидНМА -> ВидНематериальногоАктива с маппингом значений перечисления
        # В сырой БД поле ВидНМА уже в формате "Перечисление.ВидыНМА.Значение"
        source_field = "ВидНМА"
        
        if source_field in item and source_field in self.field_mapping:
            # Получаем значение перечисления из источника (уже в формате "Перечисление.Имя.Значение")
            source_enum_value = item[source_field]
            
            if source_enum_value and isinstance(source_enum_value, str) and source_enum_value.startswith("Перечисление."):
                # Получаем целевой тип из маппинга поля
                mapping = self.field_mapping[source_field]
                target_field = mapping["target_field"]
                target_type = mapping.get("target_type", "Перечисление.ВидыНематериальныхАктивов")
                source_type = mapping.get("source_type", "Перечисление.ВидыНМА")
                
                # Преобразуем значение перечисления через маппинг
                mapped_enum = self._map_enum_value(source_enum_value, source_type, target_type)
                
                # Если маппинг успешен и результат отличается от исходного, используем его
                if mapped_enum != source_enum_value and mapped_enum.startswith("Перечисление."):
                    result[target_field] = mapped_enum
                    verbose_print(f"  → Преобразовано ВидНМА '{source_enum_value}' → ВидНематериальногоАктива '{mapped_enum}'")
                elif target_type and target_type.startswith("Перечисление."):
                    # Если маппинг не найден, но тип определен, заменяем только тип перечисления
                    parts = source_enum_value.split(".", 2)
                    if len(parts) == 3:
                        enum_value = parts[2]  # Извлекаем значение
                        target_enum_name = target_type.replace("Перечисление.", "")
                        result[target_field] = f"Перечисление.{target_enum_name}.{enum_value}"
                        verbose_print(f"  → ВидНМА '{source_enum_value}' → ВидНематериальногоАктива '{result[target_field]}' (заменен тип перечисления)")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} нематериальныеактивы...")
        processed = []
        ...
        verbose_print(f"Обработано нематериальныеактивы: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "intangible_assets"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "intangible_assets")
        ...
        if not processed:
            verbose_print("Нет обработанных нематериальныеактивы для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено нематериальныеактивы в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении нематериальныеактивы: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_intangible_assets(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает нематериальныеактивы из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА НЕМАТЕРИАЛЬНЫЕАКТИВЫ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение нематериальныеактивы из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("НематериальныеАктивы не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано нематериальныеактивы: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = IntangibleAssetsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка нематериальныеактивы с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка нематериальныеактивы с маппингом")
    ...
    success = process_intangible_assets(args.source_db, args.processed_db)
