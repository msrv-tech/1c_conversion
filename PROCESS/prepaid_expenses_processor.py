# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «РасходыБудущихПериодов» с использованием маппинга типов и полей.

Читает расходыбудущихпериодов из исходной БД, применяет маппинг из type_mapping.db
и сохраняет результат в новую БД в формате приемника (UH).
"""

from __future__ import annotations

import json
import os
import sys
from typing import Dict, List

# Добавляем путь к корню проекта
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from tools.encoding_fix import fix_encoding
from tools.db_manager import connect_to_sqlite, ensure_database_exists
from tools.onec_connector import upsert_rows

from tools.base_processor import MappingProcessor
from tools.processor_utils import read_from_db, copy_tabular_sections
from tools.logger import verbose_print  # noqa: E402

fix_encoding()


class PrepaidExpensesMappingProcessor(MappingProcessor):
    """Процессор для преобразования расходыбудущихпериодов с использованием маппинга."""

    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        """
        Инициализация процессора.
        
        Args:
            mapping_db_path: Путь к базе данных с маппингом
        """
        super().__init__(mapping_db_path, "РасходыБудущихПериодов", "catalog")

    def process_item_single(self, item: Dict) -> Dict:
        """
        Преобразует элемент из формата источника в формат приемника.
        
        Args:
            item: Словарь с данными элемента из источника
            
        Returns:
            Словарь с данными элемента для приемника
        """
        processed = self.process_item(item)
        
        # Добавляем субконто для счетов из исходных данных
        # Важно: делаем это ПОСЛЕ process_item, чтобы маппинг счета уже был применен
        if processed and "СчетЗатрат" in processed:
            self._add_subconto_to_account(processed, item)
        
        return processed
    
    def _add_subconto_to_account(self, processed: Dict, source_item: Dict) -> None:
        """
        Добавляет субконто к счету на основе информации из chart_of_accounts_mapping.json.
        
        Args:
            processed: Обработанный элемент (будет изменен)
            source_item: Исходный элемент с данными из сырой БД
        """
        account_field = "СчетЗатрат"
        account_value = processed.get(account_field)
        
        if not account_value or not isinstance(account_value, str):
            return
        
        # Парсим JSON счета
        try:
            account_json = json.loads(account_value)
        except (json.JSONDecodeError, ValueError):
            return
        
        # Извлекаем код счета из presentation (например, "25.20" или "90.02")
        # presentation может содержать не только код, но и название, поэтому используем extract_account_code
        from tools.chart_of_accounts_mapper import extract_account_code, get_mapped_account_code
        presentation = account_json.get("presentation", "").strip()
        account_code = extract_account_code(presentation) if presentation else None
        if not account_code:
            return
        
        # Важно: конфигурация субконто хранится по коду приемника (target), а не по коду источника
        # Поэтому если был маппинг (например, 20.01 -> 20), нужно использовать маппированный код
        if hasattr(self, 'chart_of_accounts_mapping') and self.chart_of_accounts_mapping:
            mapped_code = get_mapped_account_code(account_code, self.chart_of_accounts_mapping)
            if mapped_code and mapped_code != account_code:
                # Используем маппированный код для поиска конфигурации
                account_code = mapped_code
        
        # Проверяем, есть ли информация о субконто для этого счета
        if not hasattr(self, 'chart_of_accounts_subconto') or not self.chart_of_accounts_subconto:
            return
        
        subconto_config = self.chart_of_accounts_subconto.get(account_code)
        if not subconto_config:
            return
        
        # Добавляем субконто в JSON счета
        subconto_data = {}
        
        # Вспомогательная функция для получения данных ссылки из source_item
        def get_reference_data(field_name: str) -> Dict:
            """Получает данные ссылки из source_item (JSON или отдельные поля)."""
            # Проверяем JSON формат
            if field_name in source_item and source_item[field_name]:
                field_value = source_item[field_name]
                if isinstance(field_value, str) and field_value.strip().startswith('{'):
                    try:
                        return json.loads(field_value)
                    except (json.JSONDecodeError, ValueError):
                        pass
            
            # Пробуем собрать из отдельных полей
            uuid_field = f"{field_name}_UUID"
            presentation_field = f"{field_name}_Представление"
            type_field = f"{field_name}_Тип"
            
            ref_uuid = source_item.get(uuid_field, "")
            ref_presentation = source_item.get(presentation_field, "")
            ref_type = source_item.get(type_field, "")
            
            if ref_uuid or ref_presentation or ref_type:
                return {
                    "uuid": ref_uuid or "",
                    "presentation": ref_presentation or "",
                    "type": ref_type or ""
                }
            
            return {}
        
        # Заполняем субконто согласно конфигурации
        for subconto_key, source_field_name in subconto_config.items():
            ref_data = get_reference_data(source_field_name)
            if ref_data and (ref_data.get("uuid") or ref_data.get("presentation")):
                subconto_data[subconto_key] = ref_data
        
        # Добавляем субконто в JSON счета временно, затем извлекаем в отдельные поля
        if subconto_data:
            # Извлекаем субконто из JSON счета и сохраняем в отдельные поля
            # СубконтоЗатрат1, СубконтоЗатрат2, СубконтоЗатрат3
            for subconto_key, subconto_value in subconto_data.items():
                # subconto_key может быть "subconto1", "subconto2", "subconto3"
                # Преобразуем в имя поля: "СубконтоЗатрат1", "СубконтоЗатрат2", "СубконтоЗатрат3"
                if subconto_key.startswith("subconto"):
                    subconto_num = subconto_key.replace("subconto", "")
                    target_field = f"СубконтоЗатрат{subconto_num}"
                    # Сохраняем субконто как JSON строку
                    processed[target_field] = json.dumps(subconto_value, ensure_ascii=False)
            
            # НЕ добавляем субконто в JSON счета - они уже в отдельных полях
            # Сохраняем только сам счет без субконто
            processed[account_field] = json.dumps(account_json, ensure_ascii=False)

    def process_items(self, items: List[Dict]) -> List[Dict]:
        """
        Преобразует список элементов.
        
        Args:
            items: Список словарей с данными элементов из источника
            
        Returns:
            Список словарей с данными элементов для приемника
        """
        verbose_print(f"\nОбработка {len(items)} расходыбудущихпериодов...")
        processed = []

        for i, item in enumerate(items, 1):
            if i % 100 == 0:
                verbose_print(f"  Обработано: {i}/{len(items)}")

            processed_item = self.process_item_single(item)
            if processed_item:
                processed.append(processed_item)

        verbose_print(f"Обработано расходыбудущихпериодов: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "prepaid_expenses"
    ) -> bool:
        """
        Преобразует элементы и сохраняет их в новую базу данных.
        
        Args:
            items: Список словарей с данными элементов из источника
            output_db_path: Путь к выходной базе данных SQLite
            table_name: Имя таблицы для сохранения (по умолчанию "prepaid_expenses")
            
        Returns:
            True если успешно, False если ошибка
        """
        # Обрабатываем элементы
        processed = self.process_items(items)
        
        if not processed:
            verbose_print("Нет обработанных расходыбудущихпериодов для сохранения")
            return False

        # Подготавливаем базу данных
        if not ensure_database_exists(output_db_path):
            verbose_print(f"Не удалось подготовить базу данных: {output_db_path}")
            return False

        connection = connect_to_sqlite(output_db_path)
        if not connection:
            verbose_print(f"Не удалось подключиться к базе данных: {output_db_path}")
            return False

        try:
            # Преобразуем ссылки в JSON формат
            reference_fields = set()
            for item in processed:
                for field_name in item.keys():
                    if field_name.endswith("_UUID") or field_name.endswith("_Представление") or field_name.endswith("_Тип"):
                        base_field = field_name.rsplit("_", 1)[0]
                        reference_fields.add(base_field)

            for item in processed:
                for ref_field in reference_fields:
                    uuid_field = f"{ref_field}_UUID"
                    presentation_field = f"{ref_field}_Представление"
                    type_field = f"{ref_field}_Тип"
                    
                    # Для поля "Родитель" проверяем наличие Родитель_ЭтоГруппа и добавляем в JSON
                    is_group_value = None
                    if ref_field == "Родитель":
                        is_group_key = f"{ref_field}_ЭтоГруппа"
                        if is_group_key in item:
                            is_group_value = item.pop(is_group_key, None)
                            # Преобразуем в булево значение
                            if isinstance(is_group_value, bool):
                                pass  # Уже булево
                            elif isinstance(is_group_value, (int, str)):
                                is_group_value = str(is_group_value).lower() in ('1', 'true', 'истина', 'да')
                            elif is_group_value is None:
                                is_group_value = False

                    if uuid_field in item and item[uuid_field]:
                        ref_uuid = item[uuid_field]
                        ref_presentation = item.get(presentation_field, "")
                        ref_type = item.get(type_field, "")
                        
                        json_data = {
                            "uuid": ref_uuid,
                            "presentation": ref_presentation,
                            "type": ref_type
                        }
                        # Добавляем is_group для поля "Родитель"
                        if is_group_value is not None:
                            json_data["is_group"] = is_group_value
                        
                        ref_json = json.dumps(json_data, ensure_ascii=False)

                        item[ref_field] = ref_json

                        item.pop(uuid_field, None)
                        item.pop(presentation_field, None)
                        item.pop(type_field, None)

            saved = upsert_rows(
                connection,
                table_name,
                processed,
                {"uuid": "TEXT PRIMARY KEY", "Ссылка": "TEXT", "Код": "TEXT", "Наименование": "TEXT", "ПометкаУдаления": "INTEGER"},
            )

            if saved:
                verbose_print(f"\nСохранено расходыбудущихпериодов в БД: {len(processed)}")
                verbose_print(f"База данных: {output_db_path}")
                verbose_print(f"Таблица: {table_name}")

            connection.commit()
            return saved

        except Exception as error:
            print(f"Ошибка при сохранении расходыбудущихпериодов: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
        finally:
            connection.close()


def process_prepaid_expenses(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает расходыбудущихпериодов из исходной БД и сохраняет в новую БД.
    
    Args:
        source_db_path: Путь к исходной базе данных SQLite
        processed_db_path: Путь к выходной базе данных SQLite
        
    Returns:
        True если успешно, False если ошибка
    """
    mapping_db_path = "CONF/type_mapping.db"
    table_name = "prepaid_expenses"
    
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА РАСХОДЫБУДУЩИХПЕРИОДОВ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение расходыбудущихпериодов из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("РасходыБудущихПериодов не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано расходыбудущихпериодов: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = PrepaidExpensesMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка расходыбудущихпериодов с использованием маппинга...")
    success = processor.process_and_save_items(items, processed_db_path, table_name)

    if success:
        verbose_print("\n[4/5] Копирование табличных частей...")
        copy_tabular_sections(source_db_path, processed_db_path, table_name)

        verbose_print("\n[5/5] Обработка завершена успешно!")
        verbose_print(f"Результат сохранен в: {processed_db_path}")

    return success


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Обработка расходыбудущихпериодов с маппингом")
    parser.add_argument("--source-db", required=True, help="Путь к исходной БД")
    parser.add_argument("--processed-db", required=True, help="Путь к обработанной БД")

    args = parser.parse_args()
    success = process_prepaid_expenses(args.source_db, args.processed_db)
    sys.exit(0 if success else 1)
