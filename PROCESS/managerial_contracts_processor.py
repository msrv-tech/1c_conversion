# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «УправленческиеДоговоры» с использованием маппинга типов и полей.

Читает управленческиедоговоры из исходной БД, применяет маппинг из type_mapping.db
...
class ManagerialContractsMappingProcessor(MappingProcessor):
    """Процессор для преобразования управленческихдоговоров с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "УправленческиеДоговоры", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} управленческихдоговоров...")
        processed = []
        ...
        verbose_print(f"Обработано управленческихдоговоров: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "managerial_contracts"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "managerial_contracts")
        ...
        if not processed:
            verbose_print("Нет обработанных управленческихдоговоров для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено управленческихдоговоров в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении управленческихдоговоров: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_managerial_contracts(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает управленческиедоговоры из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА УПРАВЛЕНЧЕСКИХДОГОВОРОВ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение управленческихдоговоров из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("УправленческиеДоговоры не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано управленческихдоговоров: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = ManagerialContractsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка управленческихдоговоров с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка управленческихдоговоров с маппингом")
    ...
    success = process_managerial_contracts(args.source_db, args.processed_db)
