# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «Сотрудники» с использованием маппинга типов и полей.

Читает сотрудники из исходной БД, применяет маппинг из type_mapping.db
...
class EmployeesMappingProcessor(MappingProcessor):
    """Процессор для преобразования сотрудники с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "СотрудникиОрганизаций", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} сотрудники...")
        processed = []
        ...
        verbose_print(f"Обработано сотрудники: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "employees"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "employees")
        ...
        if not processed:
            verbose_print("Нет обработанных сотрудники для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено сотрудники в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении сотрудники: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_employees(
    ...
    """
    Обрабатывает сотрудники из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА СОТРУДНИКИ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение сотрудники из исходной БД: {source_db_path}")
    # Используем правильное имя таблицы из загрузчика
    source_table_name = "organization_employees"
    items = read_from_db(source_db_path, source_table_name)
    if not items:
        verbose_print("Сотрудники не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано сотрудники: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = EmployeesMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка сотрудники с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка сотрудники с маппингом")
    ...
    success = process_employees(args.source_db, args.processed_db)
