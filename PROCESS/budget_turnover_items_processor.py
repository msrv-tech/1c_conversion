# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «СтатьиОборотовПоБюджетам» с использованием маппинга типов и полей.

Читает статьиоборотовпобюджетам из исходной БД, применяет маппинг из type_mapping.db
...
class BudgetTurnoverItemsMappingProcessor(MappingProcessor):
    """Процессор для преобразования статьиоборотовпобюджетам с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "СтатьиОборотовПоБюджетам", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} статьиоборотовпобюджетам...")
        processed = []
        ...
        verbose_print(f"Обработано статьиоборотовпобюджетам: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "budget_turnover_items"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "budget_turnover_items")
        ...
        if not processed:
            verbose_print("Нет обработанных статьиоборотовпобюджетам для сохранения")
            return False
        ...
        try:
            ...
            for item in processed:
                for ref_field in reference_fields:
                    ...
                    # Проверяем, есть ли уже JSON в поле (из исходной БД)
                    existing_json = None
                    ...
                    # Проверяем наличие UUID или других метаданных
                    ref_uuid = item.get(uuid_field, "")
                    ref_presentation = item.get(presentation_field, "")
                    ref_type = item.get(type_field, "")
                    
                    # Применяем маппинг типа, если есть
                    original_type = ref_type
                    if ref_type:
                        # Пробуем найти маппинг для типа с "Ссылка" и без
                        if ref_type in self.type_mapping:
                            ref_type = self.type_mapping[ref_type]
                        elif ref_type.startswith("Справочник.") and not ref_type.startswith("СправочникСсылка."):
                            # Пробуем найти маппинг для типа с "Ссылка"
                            ref_type_with_link = ref_type.replace("Справочник.", "СправочникСсылка.", 1)
                            if ref_type_with_link in self.type_mapping:
                                ref_type = self.type_mapping[ref_type_with_link]
                            # Если не нашли, пробуем найти маппинг для типа без "Ссылка"
                            elif ref_type in self.type_mapping:
                                ref_type = self.type_mapping[ref_type]
                        
                        # Если тип начинается с "Справочник.", но маппинг не найден,
                        # пробуем найти маппинг для типа с "Ссылка" в конце
                        if ref_type == original_type and ref_type.startswith("Справочник."):
                            # Пробуем разные варианты
                            variants = [
                                ref_type.replace("Справочник.", "СправочникСсылка.", 1),
                                ref_type.replace("Справочник.", "СправочникСсылка.", 1).replace(".", "Ссылка.", 1) if "." in ref_type else None,
                            ]
                            for variant in variants:
                                if variant and variant in self.type_mapping:
                                    ref_type = self.type_mapping[variant]
                                    break
                    
                    # Создаем JSON, если есть хотя бы одно из полей (UUID, presentation или type)
                    if ref_uuid or ref_presentation or ref_type:
                        ...
                        # Если нет метаданных, устанавливаем пустую строку
                        item[ref_field] = ""
            ...
            if saved:
                verbose_print(f"\nСохранено статьиоборотовпобюджетам в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении статьиоборотовпобюджетам: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_budget_turnover_items(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает статьиоборотовпобюджетам из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА СТАТЬИОБОРОТОВПОБЮДЖЕТАМ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение статьиоборотовпобюджетам из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("СтатьиОборотовПоБюджетам не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано статьиоборотовпобюджетам: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = BudgetTurnoverItemsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка статьиоборотовпобюджетам с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка статьиоборотовпобюджетам с маппингом")
    ...
    success = process_budget_turnover_items(args.source_db, args.processed_db)
