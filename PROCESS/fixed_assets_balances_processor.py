# -*- coding: utf-8 -*-
"""
Модуль обработки остатков ОС.
На текущем этапе выполняет простое копирование данных из загруженной БД в обработанную,
подготавливая структуру для писателя. Использует маппинг значений перечислений из
type_mapping.db (enumeration_value_mapping) для ГруппыОС, СпособыПоступленияАктивов,
СпособыНачисленияАмортизацииОС и др.
"""
...
def process_fixed_assets_balances(source_db_path: str, processed_db_path: str) -> bool:
    """
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ОСТАТКОВ ОС")
    verbose_print("=" * 80)
    ...
    # Загружаем маппинг значений перечислений из type_mapping.db
    enum_value_mapping = _load_enum_value_mapping(MAPPING_DB)
    enum_src_postup = "Перечисление.СпособыПоступленияАктивов"
    enum_src_amort = "Перечисление.СпособыНачисленияАмортизацииОС"
    enum_src_gruppa_os = "Перечисление.ГруппыОС"
    ...
    def _to_full_enum(enum_type: str, value: str) -> str:
        """Форматирует значение в вид Перечисление.ИмяПеречисления.Значение."""
        if not value:
            return ""
    ...
    def _map_enum(val: str, enum_type: str) -> Optional[str]:
        """Ищет маппинг и возвращает полный формат Перечисление.Имя.Значение."""
        ...
        if not mapped:
            mapped = short_val  # Без маппинга — оставляем как есть
        return _to_full_enum(enum_type, mapped)

    for item in items:
        # СпособПоступленияРегл (СпособыПоступленияАктивов) — в формате Перечисление.Имя.Значение
        val = item.get("СпособПоступленияРегл")
        if val and isinstance(val, str):
            full_val = _map_enum(val, enum_src_postup)
            if full_val:
                item["СпособПоступленияРегл"] = full_val
                short_orig = val.split(".")[-1] if "." in val else val
                short_new = full_val.split(".")[-1]
                if short_new != short_orig:
                    verbose_print(f"  → СпособПоступленияРегл: {val} → {full_val}")

        # СпособНачисленияАмортизацииБУ (СпособыНачисленияАмортизацииОС) — в формате Перечисление.Имя.Значение
        ...
        if val and isinstance(val, str):
            ...
            if full_val:
                ...
                short_new = full_val.split(".")[-1]
                if short_new != short_orig:
                    verbose_print(f"  → СпособНачисленияАмортизацииБУ: {val} → {full_val}")

        # ГруппаОС (ГруппыОС): УПП имеет customИнструмент, УХ — нет; маппинг в enumeration_value_mapping
        val = item.get("ГруппаОС")
        if val and isinstance(val, str):
            full_val = _map_enum(val, enum_src_gruppa_os)
            if full_val:
                item["ГруппаОС"] = full_val
                short_orig = val.split(".")[-1] if "." in val else val
                short_new = full_val.split(".")[-1]
                if short_new != short_orig:
                    verbose_print(f"  → ГруппаОС: {val} → {full_val}")

        # МетодНачисленияАмортизацииНУ, ПорядокВключенияСтоимостиВСоставРасходовНУ, ПорядокВключенияСтоимостиВСоставРасходовУСН
        ...
        # ПорядокПогашенияСтоимостиБУ — маппинг из ПорядокВключенияСтоимостиВСоставРасходовНУ (УПП) в ПорядокПогашенияСтоимостиОС (УХ)
        # По правилам конвертации (Правила_УПП-УХ.xml): НачислениеАмортизации/ВключениеВРасходы → НачислениеАмортизации, иначе СтоимостьНеВключается → СтоимостьНеПогашается
        ORDER_IN_NU_TO_ORDER_PAYMENT_BU = {
            "НачислениеАмортизации": "НачислениеАмортизации",
            "ВключениеВРасходыПриПринятииКУчету": "НачислениеАмортизации",  # по правилам
        ...
        if val_order and isinstance(val_order, str):
            ...
            mapped = ORDER_IN_NU_TO_ORDER_PAYMENT_BU.get(short_val, "НачислениеАмортизации")
        else:
    ...
    try:
        # Определяем типы колонок для сохранения
        # Числовые колонки должны быть REAL
        column_types = {"uuid": "TEXT PRIMARY KEY"}
        
        # Список числовых полей из загрузчика
        numeric_columns = [
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Обработка остатков ОС")
    parser.add_argument("--source-db", default="BD/fixed_assets_balances.db", help="Путь к исходной БД")
    parser.add_argument("--processed-db", default="BD/fixed_assets_balances_processed.db", help="Путь к обработанной БД")

    args = parser.parse_args()
    process_fixed_assets_balances(args.source_db, args.processed_db)
