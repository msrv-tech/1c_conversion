# -*- coding: utf-8 -*-
"""
Модуль обработки остатков взаиморасчетов с поставщиками.
"""
...
SOURCE_TABLE = "supplier_balances"
PROCESSED_TABLE = "supplier_balances"

def process_supplier_balances(source_db_path: str, processed_db_path: str) -> bool:
    """Обработка данных об остатках для записи в документ."""
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ОСТАТКОВ ВЗАИМОРАСЧЕТОВ С ПОСТАВЩИКАМИ")
    verbose_print("=" * 80)
...
if __name__ == "__main__":
    process_supplier_balances(
        "BD/supplier_balances.db",
        "BD/supplier_balances_processed.db"
    )
