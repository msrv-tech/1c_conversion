# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «БанковскиеСчета» с использованием маппинга типов и полей.

Читает банковскиесчета из исходной БД, применяет маппинг из type_mapping.db
...
class BankAccountsMappingProcessor(MappingProcessor):
    """Процессор для преобразования банковскиесчета с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "БанковскиеСчета", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} банковскиесчета...")
        processed = []
        ...
        verbose_print(f"Обработано банковскиесчета: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "bank_accounts"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "bank_accounts")
        ...
        if not processed:
            verbose_print("Нет обработанных банковскиесчета для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено банковскиесчета в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении банковскиесчета: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_bank_accounts(
    ...
    """
    Обрабатывает банковскиесчета из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА БАНКОВСКИЕСЧЕТА С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение банковскиесчета из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("БанковскиеСчета не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано банковскиесчета: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = BankAccountsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка банковскиесчета с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка банковскиесчета с маппингом")
    ...
    success = process_bank_accounts(args.source_db, args.processed_db)
