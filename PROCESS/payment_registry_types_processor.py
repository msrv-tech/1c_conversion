# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «custom_ВидыРеестраПлатежейCUSTOM» с использованием маппинга типов и полей.

Читает custom_видыреестраплатежейcustom из исходной БД, применяет маппинг из type_mapping.db
...
class PaymentRegistryTypesMappingProcessor(MappingProcessor):
    """Процессор для преобразования custom_видыреестраплатежейcustom с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "custom_ВидыРеестраПлатежейCUSTOM", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} custom_видыреестраплатежейcustom...")
        processed = []
        ...
        verbose_print(f"Обработано custom_видыреестраплатежейcustom: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "payment_registry_types"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "payment_registry_types")
        ...
        if not processed:
            verbose_print("Нет обработанных custom_видыреестраплатежейcustom для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено custom_видыреестраплатежейcustom в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении custom_видыреестраплатежейcustom: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_payment_registry_types(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает custom_видыреестраплатежейcustom из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА CUSTOM_ВИДЫРЕЕСТРАПЛАТЕЖЕЙCUSTOM С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение custom_видыреестраплатежейcustom из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("custom_ВидыРеестраПлатежейCUSTOM не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано custom_видыреестраплатежейcustom: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = PaymentRegistryTypesMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка custom_видыреестраплатежейcustom с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка custom_видыреестраплатежейcustom с маппингом")
    ...
    success = process_payment_registry_types(args.source_db, args.processed_db)
