# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «СобытияОС» с использованием маппинга типов и полей.

Читает событияос из исходной БД, применяет маппинг из type_mapping.db
...
class FixedAssetEventsMappingProcessor(MappingProcessor):
    """Процессор для преобразования событияос с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "СобытияОС", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        verbose_print(f"Обработано событияос: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "fixed_asset_events"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "fixed_asset_events")
        ...
        if not processed:
            verbose_print("Нет обработанных событияос для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено событияос в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении событияос: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_fixed_asset_events(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает событияос из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА СОБЫТИЯОС С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение событияос из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("СобытияОС не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано событияос: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = FixedAssetEventsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка событияос с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка событияос с маппингом")
    ...
    success = process_fixed_asset_events(args.source_db, args.processed_db)
