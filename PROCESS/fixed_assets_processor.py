# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «ОсновныеСредства» с использованием маппинга типов и полей.

Читает основныесредства из исходной БД, применяет маппинг из type_mapping.db
и сохраняет результат в новую БД в формате приемника (UH).
"""

from __future__ import annotations

import json
import os
import sys
from typing import Dict, List

# Добавляем путь к корню проекта
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))

from tools.encoding_fix import fix_encoding
from tools.db_manager import connect_to_sqlite, ensure_database_exists
from tools.onec_connector import upsert_rows

from tools.base_processor import MappingProcessor
from tools.processor_utils import read_from_db, copy_tabular_sections
from tools.logger import verbose_print  # noqa: E402

fix_encoding()


class FixedAssetsMappingProcessor(MappingProcessor):
    """Процессор для преобразования основныесредства с использованием маппинга."""

    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        """
        Инициализация процессора.
        
        Args:
            mapping_db_path: Путь к базе данных с маппингом
        """
        super().__init__(mapping_db_path, "ОсновныеСредства", "catalog")

    def process_item_single(self, item: Dict) -> Dict:
        """
        Преобразует элемент из формата источника в формат приемника.
        
        Args:
            item: Словарь с данными элемента из источника
            
        Returns:
            Словарь с данными элемента для приемника
        """
        return self.process_item(item)

    def process_items(self, items: List[Dict]) -> List[Dict]:
        """
        Преобразует список элементов.
        
        Args:
            items: Список словарей с данными элементов из источника
            
        Returns:
            Список словарей с данными элементов для приемника
        """
        verbose_print(f"\nОбработка {len(items)} основныесредства...")
        processed = []

        for i, item in enumerate(items, 1):
            if i % 100 == 0:
                verbose_print(f"  Обработано: {i}/{len(items)}")

            processed_item = self.process_item_single(item)
            if processed_item:
                processed.append(processed_item)

        verbose_print(f"Обработано основныесредства: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "fixed_assets"
    ) -> bool:
        """
        Преобразует элементы и сохраняет их в новую базу данных.
        
        Args:
            items: Список словарей с данными элементов из источника
            output_db_path: Путь к выходной базе данных SQLite
            table_name: Имя таблицы для сохранения (по умолчанию "fixed_assets")
            
        Returns:
            True если успешно, False если ошибка
        """
        # Обрабатываем элементы
        processed = self.process_items(items)
        
        if not processed:
            verbose_print("Нет обработанных основныесредства для сохранения")
            return False

        # Подготавливаем базу данных
        if not ensure_database_exists(output_db_path):
            verbose_print(f"Не удалось подготовить базу данных: {output_db_path}")
            return False

        connection = connect_to_sqlite(output_db_path)
        if not connection:
            verbose_print(f"Не удалось подключиться к базе данных: {output_db_path}")
            return False

        try:
            # Вспомогательная функция: получить search_method для ref_field из field_mapping
            def _get_search_method(ref_field: str) -> str | None:
                for src, m in self.field_mapping.items():
                    if m.get("target_field") == ref_field:
                        return m.get("search_method")
                return None

            # Преобразуем ссылки в JSON формат
            reference_fields = set()
            for item in processed:
                for field_name in item.keys():
                    if field_name.endswith("_UUID") or field_name.endswith("_Представление") or field_name.endswith("_Тип"):
                        base_field = field_name.rsplit("_", 1)[0]
                        reference_fields.add(base_field)

            for item in processed:
                for ref_field in reference_fields:
                    uuid_field = f"{ref_field}_UUID"
                    presentation_field = f"{ref_field}_Представление"
                    type_field = f"{ref_field}_Тип"
                    
                    # Для поля "Родитель" проверяем наличие Родитель_ЭтоГруппа и добавляем в JSON
                    is_group_value = None
                    if ref_field == "Родитель":
                        is_group_key = f"{ref_field}_ЭтоГруппа"
                        if is_group_key in item:
                            is_group_value = item.pop(is_group_key, None)
                            # Преобразуем в булево значение
                            if isinstance(is_group_value, bool):
                                pass  # Уже булево
                            elif isinstance(is_group_value, (int, str)):
                                is_group_value = str(is_group_value).lower() in ('1', 'true', 'истина', 'да')
                            elif is_group_value is None:
                                is_group_value = False

                    # Конвертируем при наличии любого из полей (в т.ч. при пустом UUID для string_to_reference)
                    has_ref_meta = (
                        (uuid_field in item) or
                        (presentation_field in item) or
                        (type_field in item)
                    )
                    if has_ref_meta:
                        ref_uuid = item.get(uuid_field, "") or ""
                        ref_presentation = item.get(presentation_field, "") or ""
                        ref_type = item.get(type_field, "") or ""
                        
                        json_data = {
                            "uuid": ref_uuid,
                            "presentation": ref_presentation,
                            "type": ref_type
                        }
                        # Добавляем is_group для поля "Родитель"
                        if is_group_value is not None:
                            json_data["is_group"] = is_group_value
                        
                        # Добавляем search_method из маппинга
                        search_method = _get_search_method(ref_field)
                        if search_method:
                            json_data["search_method"] = search_method
                            if search_method == "reference_by_code":
                                code_field = f"{ref_field}_Код"
                                code_value = (item.get(code_field, "") or "").strip()
                                if code_value:
                                    json_data["presentation"] = str(code_value)
                        
                        ref_json = json.dumps(json_data, ensure_ascii=False)
                        item[ref_field] = ref_json

                        item.pop(uuid_field, None)
                        item.pop(presentation_field, None)
                        item.pop(type_field, None)

            # Отдельный проход: для ref-полей, уже пришедших как JSON (напр. КодПоОКОФ)
            for item in processed:
                for ref_field in list(item.keys()):
                    val = item.get(ref_field)
                    if not isinstance(val, str) or not val.strip().startswith("{"):
                        continue
                    try:
                        ref_json = json.loads(val)
                    except json.JSONDecodeError:
                        continue
                    if not isinstance(ref_json, dict) or "type" not in ref_json:
                        continue
                    search_method = _get_search_method(ref_field)
                    if not search_method:
                        continue
                    ref_json["search_method"] = search_method
                    if search_method == "reference_by_code":
                        code_field = f"{ref_field}_Код"
                        code_value = (item.get(code_field, "") or "").strip()
                        if code_value:
                            ref_json["presentation"] = str(code_value)
                    item[ref_field] = json.dumps(ref_json, ensure_ascii=False)

            saved = upsert_rows(
                connection,
                table_name,
                processed,
                {"uuid": "TEXT PRIMARY KEY", "Ссылка": "TEXT", "Код": "TEXT", "Наименование": "TEXT", "ПометкаУдаления": "INTEGER"},
            )

            if saved:
                verbose_print(f"\nСохранено основныесредства в БД: {len(processed)}")
                verbose_print(f"База данных: {output_db_path}")
                verbose_print(f"Таблица: {table_name}")

            connection.commit()
            return saved

        except Exception as error:
            print(f"Ошибка при сохранении основныесредства: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
        finally:
            connection.close()


def process_fixed_assets(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает основныесредства из исходной БД и сохраняет в новую БД.
    
    Args:
        source_db_path: Путь к исходной базе данных SQLite
        processed_db_path: Путь к выходной базе данных SQLite
        
    Returns:
        True если успешно, False если ошибка
    """
    mapping_db_path = "CONF/type_mapping.db"
    table_name = "fixed_assets"
    
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ОСНОВНЫЕСРЕДСТВА С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение основныесредства из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("ОсновныеСредства не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано основныесредства: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = FixedAssetsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка основныесредства с использованием маппинга...")
    success = processor.process_and_save_items(items, processed_db_path, table_name)

    if success:
        verbose_print("\n[4/5] Копирование табличных частей...")
        copy_tabular_sections(source_db_path, processed_db_path, table_name)

        verbose_print("\n[5/5] Обработка завершена успешно!")
        verbose_print(f"Результат сохранен в: {processed_db_path}")

    return success


if __name__ == "__main__":
    import argparse

    parser = argparse.ArgumentParser(description="Обработка основныесредства с маппингом")
    parser.add_argument("--source-db", required=True, help="Путь к исходной БД")
    parser.add_argument("--processed-db", required=True, help="Путь к обработанной БД")

    args = parser.parse_args()
    success = process_fixed_assets(args.source_db, args.processed_db)
    sys.exit(0 if success else 1)
