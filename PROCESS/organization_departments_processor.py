# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «ПодразделенияОрганизаций» с использованием маппинга типов и полей.

Читает подразделенияорганизаций из исходной БД, применяет маппинг из type_mapping.db
...
class OrganizationDepartmentsMappingProcessor(MappingProcessor):
    """Процессор для преобразования подразделенияорганизаций с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "ПодразделенияОрганизаций", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} подразделенияорганизаций...")
        processed = []
        ...
        verbose_print(f"Обработано подразделенияорганизаций: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "organization_departments"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "organization_departments")
        ...
        if not processed:
            verbose_print("Нет обработанных подразделенияорганизаций для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено подразделенияорганизаций в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении подразделенияорганизаций: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_organization_departments(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает подразделенияорганизаций из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ПОДРАЗДЕЛЕНИЯОРГАНИЗАЦИЙ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение подразделенияорганизаций из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("ПодразделенияОрганизаций не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано подразделенияорганизаций: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = OrganizationDepartmentsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка подразделенияорганизаций с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка подразделенияорганизаций с маппингом")
    ...
    success = process_organization_departments(args.source_db, args.processed_db)
