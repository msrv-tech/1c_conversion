# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «ПрочиеДоходыИРасходы» с использованием маппинга типов и полей.

Читает прочиедоходыирасходы из исходной БД, применяет маппинг из type_mapping.db
...
class OtherIncomeAndExpensesMappingProcessor(MappingProcessor):
    """Процессор для преобразования прочиедоходыирасходы с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "ПрочиеДоходыИРасходы", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} прочиедоходыирасходы...")
        processed = []
        ...
        verbose_print(f"Обработано прочиедоходыирасходы: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "other_income_and_expenses"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "other_income_and_expenses")
        ...
        if not processed:
            verbose_print("Нет обработанных прочиедоходыирасходы для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено прочиедоходыирасходы в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении прочиедоходыирасходы: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_other_income_and_expenses(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает прочиедоходыирасходы из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ПРОЧИЕДОХОДЫИРАСХОДЫ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение прочиедоходыирасходы из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("ПрочиеДоходыИРасходы не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано прочиедоходыирасходы: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = OtherIncomeAndExpensesMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка прочиедоходыирасходы с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка прочиедоходыирасходы с маппингом")
    ...
    success = process_other_income_and_expenses(args.source_db, args.processed_db)
