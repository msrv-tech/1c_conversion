# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «ФизическиеЛица» с использованием маппинга типов и полей.

Читает физическиелица из исходной БД, применяет маппинг из type_mapping.db
...
class IndividualPersonsMappingProcessor(MappingProcessor):
    """Процессор для преобразования физическиелица с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        super().__init__(mapping_db_path, "ФизическиеЛица", "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} физическиелица...")
        processed = []
        ...
        verbose_print(f"Обработано физическиелица: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "individual_persons"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "individual_persons")
        ...
        if not processed:
            verbose_print("Нет обработанных физическиелица для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено физическиелица в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении физическиелица: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_individual_persons(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает физическиелица из исходной БД и сохраняет в новую БД.
    ...
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА ФИЗИЧЕСКИЕЛИЦА С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение физическиелица из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("ФизическиеЛица не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано физическиелица: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = IndividualPersonsMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка физическиелица с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка физическиелица с маппингом")
    ...
    success = process_individual_persons(args.source_db, args.processed_db)
