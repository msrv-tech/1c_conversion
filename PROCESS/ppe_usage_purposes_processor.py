# -*- coding: utf-8 -*-
"""
Модуль обработки справочника «НазначенияИспользованияНоменклатуры» с использованием маппинга типов и полей.

Читает назначенияиспользованияноменклатуры из исходной БД, применяет маппинг из type_mapping.db
...
class PpeUsagePurposesMappingProcessor(MappingProcessor):
    """Процессор для преобразования назначенияиспользованияноменклатуры с использованием маппинга."""
    ...
    def __init__(self, mapping_db_path: str = "CONF/type_mapping.db"):
        ...
        """
        # Определяем исходное имя объекта через маппинг объектов
        import sqlite3
        source_object_name = "НазначенияИспользования"  # По умолчанию
        if os.path.exists(mapping_db_path):
            try:
                ...
                cursor.execute("""
                    SELECT source_name
                    FROM object_mapping
                    WHERE object_type = ? AND target_name = ?
                """, ("catalog", "НазначенияИспользованияНоменклатуры"))
                row = cursor.fetchone()
                if row:
                    source_object_name = row["source_name"]
                conn.close()
            except Exception:
                pass  # Используем значение по умолчанию
        
        # Используем исходное имя для загрузки маппинга полей
        super().__init__(mapping_db_path, source_object_name, "catalog")
    ...
    def process_items(self, items: List[Dict]) -> List[Dict]:
        ...
        """
        verbose_print(f"\nОбработка {len(items)} назначенияиспользованияноменклатуры...")
        processed = []
        ...
        verbose_print(f"Обработано назначенияиспользованияноменклатуры: {len(processed)}")
        return processed

    def process_and_save_items(
        self, items: List[Dict], output_db_path: str, table_name: str = "ppe_usage_purposes"
    ) -> bool:
        """
        ...
        Args:
            ...
            table_name: Имя таблицы для сохранения (по умолчанию "ppe_usage_purposes")
        ...
        if not processed:
            verbose_print("Нет обработанных назначенияиспользованияноменклатуры для сохранения")
            return False
        ...
        try:
            ...
            if saved:
                verbose_print(f"\nСохранено назначенияиспользованияноменклатуры в БД: {len(processed)}")
            ...
            print(f"Ошибка при сохранении назначенияиспользованияноменклатуры: {error}")
            import traceback
            traceback.print_exc()
            connection.rollback()
            return False
...
def process_ppe_usage_purposes(
    source_db_path: str,
    processed_db_path: str,
) -> bool:
    """
    Обрабатывает назначенияиспользованияноменклатуры из исходной БД и сохраняет в новую БД.
    ...
    table_name = "ppe_usage_purposes"
    
    verbose_print("=" * 80)
    verbose_print("ОБРАБОТКА НАЗНАЧЕНИЯИСПОЛЬЗОВАНИЯНОМЕНКЛАТУРЫ С МАППИНГОМ")
    verbose_print("=" * 80)

    # Читаем данные из исходной БД
    verbose_print(f"\n[1/5] Чтение назначенияиспользованияноменклатуры из исходной БД: {source_db_path}")
    items = read_from_db(source_db_path, table_name)
    if not items:
        verbose_print("НазначенияИспользованияНоменклатуры не найдены в исходной БД")
        return False
    verbose_print(f"Прочитано назначенияиспользованияноменклатуры: {len(items)}")

    # Инициализируем процессор
    verbose_print("\n[2/5] Инициализация процессора маппинга...")
    processor = PpeUsagePurposesMappingProcessor(mapping_db_path)

    # Обрабатываем данные
    verbose_print("\n[3/5] Обработка назначенияиспользованияноменклатуры с использованием маппинга...")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Обработка назначенияиспользованияноменклатуры с маппингом")
    ...
    success = process_ppe_usage_purposes(args.source_db, args.processed_db)
