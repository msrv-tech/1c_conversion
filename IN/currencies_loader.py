# -*- coding: utf-8 -*-
"""
Загрузка справочника «Валюты» из 1С в SQLite.
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    Каталог.ПараметрыПрописиНаРусском КАК ПараметрыПрописиНаРусском
ИЗ
    Справочник.Валюты КАК Каталог
"""
...
def load_currencies(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «ВАЛЮТЫ»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Валюты» не найдены.")
