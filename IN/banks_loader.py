# -*- coding: utf-8 -*-
"""
Загрузка справочника «Банки» из 1С в SQLite.
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    Каталог.КоррСчет КАК КоррСчет,
    Каталог.Город КАК Город,
    Каталог.Адрес КАК Адрес,
    Каталог.Телефоны КАК Телефоны,
    Каталог.БИКРКЦ КАК БИКРКЦ,
    ПРЕДСТАВЛЕНИЕ(Каталог.БИКРКЦ) КАК БИКРКЦ_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.БИКРКЦ)) КАК БИКРКЦ_UUID,
    ВЫБОР
        КОГДА Каталог.БИКРКЦ = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.БИКРКЦ)
    КОНЕЦ КАК БИКРКЦ_Тип,
    Каталог.СВИФТБИК КАК СВИФТБИК,
    Каталог.МеждународноеНаименование КАК МеждународноеНаименование,
    Каталог.ГородМеждународный КАК ГородМеждународный,
    Каталог.АдресМеждународный КАК АдресМеждународный,
    Каталог.ИНН КАК ИНН,
    Каталог.ВнутреннийКодЦБ КАК ВнутреннийКодЦБ
ИЗ
    Справочник.Банки КАК Каталог
"""
...
def load_banks(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «БАНКИ»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Банки» не найдены.")
