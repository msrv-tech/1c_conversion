# -*- coding: utf-8 -*-
"""
Загрузка справочника «Регистрация в ИФНС» из 1С в SQLite.
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    Каталог.КПП КАК КПП,
    Каталог.НаименованиеИФНС КАК НаименованиеИФНС,
    Каталог.Представитель КАК Представитель,
    ПРЕДСТАВЛЕНИЕ(Каталог.Представитель) КАК Представитель_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.Представитель)) КАК Представитель_UUID,
    ВЫБОР
        КОГДА Каталог.Представитель = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.Представитель)
    КОНЕЦ КАК Представитель_Тип,
    Каталог.ДокументПредставителя КАК ДокументПредставителя,
    Каталог.УполномоченноеЛицоПредставителя КАК УполномоченноеЛицоПредставителя,
    Каталог.Доверенность КАК Доверенность,
    ПРЕДСТАВЛЕНИЕ(Каталог.Доверенность) КАК Доверенность_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.Доверенность)) КАК Доверенность_UUID,
    ВЫБОР
        КОГДА Каталог.Доверенность = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.Доверенность)
    КОНЕЦ КАК Доверенность_Тип
ИЗ
    Справочник.РегистрацияВИФНС КАК Каталог
"""
...
MAIN_QUERY_COLUMNS: List[str] = [
    ...
    "НаименованиеИФНС",
    "Представитель",
    "Представитель_Представление",
    "Представитель_UUID",
    "Представитель_Тип",
    "ДокументПредставителя",
    "УполномоченноеЛицоПредставителя",
    "Доверенность",
    "Доверенность_Представление",
    "Доверенность_UUID",
    "Доверенность_Тип",
]
...
def load_tax_registrations(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «РЕГИСТРАЦИЯ В ИФНС»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Регистрация в ИФНС» не найдены.")
    ...
    try:
        ...
        saved = upsert_rows(
            ...
            rows,
            {
                ...
                "Владелец": "TEXT",
                "КПП": "TEXT",
                "НаименованиеИФНС": "TEXT",
                "ДокументПредставителя": "TEXT",
                "УполномоченноеЛицоПредставителя": "TEXT",
            },
        )
