# -*- coding: utf-8 -*-
"""
Загрузка справочника «Контактные лица» из 1С в SQLite.

Запрос получен с помощью `tools/generate_1c_query.py`.
"""
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    Каталог.Фамилия КАК Фамилия,
    Каталог.Имя КАК Имя,
    Каталог.КоличествоДнейДоНапоминания КАК КоличествоДнейДоНапоминания,
    Каталог.НапоминатьОДнеРождения КАК НапоминатьОДнеРождения,
    Каталог.Отчество КАК Отчество,
    ...
    КОНЕЦ КАК Пол_Тип
ИЗ
    Справочник.КонтактныеЛица КАК Каталог
"""
...
REFERENCE_COLUMNS: List[str] = ["Пол"]
...
def load_contact_persons(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «КОНТАКТНЫЕ ЛИЦА»")
    _log("=" * 80)
    ...
    filter_uuids = get_catalog_uuids(filters_db, "contact_persons")
    if filter_uuids:
