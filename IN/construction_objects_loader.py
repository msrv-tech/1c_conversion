# -*- coding: utf-8 -*-
"""
Загрузка справочника «Объекты строительства» из 1С в SQLite.
...
ИЗ
    Справочник.ОбъектыСтроительства КАК Каталог
"""
...
def load_construction_objects(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «ОБЪЕКТЫ СТРОИТЕЛЬСТВА»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Объекты строительства» не найдены.")
