# -*- coding: utf-8 -*-
"""
Загрузка справочника «Статьи движения денежных средств» из 1С в SQLite.
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    Каталог.ВидДвиженияДенежныхСредств КАК ВидДвиженияДенежныхСредств,
    ПРЕДСТАВЛЕНИЕ(Каталог.ВидДвиженияДенежныхСредств) КАК ВидДвиженияДенежныхСредств_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.ВидДвиженияДенежныхСредств)) КАК ВидДвиженияДенежныхСредств_UUID,
    ВЫБОР
        КОГДА Каталог.ВидДвиженияДенежныхСредств = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.ВидДвиженияДенежныхСредств)
    КОНЕЦ КАК ВидДвиженияДенежныхСредств_Тип,
    ...
    Каталог.customКодСтатьиДДС КАК customКодСтатьиДДС
ИЗ
    Справочник.СтатьиДвиженияДенежныхСредств КАК Каталог
"""
...
MAIN_QUERY_COLUMNS: List[str] = [
    ...
    "ВидДвиженияДенежныхСредств_Представление",
    "ВидДвиженияДенежныхСредств_UUID",
    "ВидДвиженияДенежныхСредств_Тип",
    "customСтатьяБДР2020",
...
def load_cash_flow_items(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «СТАТЬИ ДВИЖЕНИЯ ДЕНЕЖНЫХ СРЕДСТВ»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Статьи движения денежных средств» не найдены.")
