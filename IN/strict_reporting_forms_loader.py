# -*- coding: utf-8 -*-
"""
Загрузка справочника «Бланки строгой отчётности» из 1С в SQLite.
...
ИЗ
    Справочник.БланкиСтрогойОтчетности КАК Каталог
"""
...
def load_strict_reporting_forms(
    sqlite_db_file: str,
    com_object,
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «БЛАНКИ СТРОГОЙ ОТЧЁТНОСТИ»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Бланки строгой отчётности» не найдены.")
