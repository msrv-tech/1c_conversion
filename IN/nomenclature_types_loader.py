# -*- coding: utf-8 -*-
"""
Загрузка справочника «Виды номенклатуры» из 1С в SQLite.
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    ПРЕДСТАВЛЕНИЕ(Каталог.ТипНоменклатуры) КАК ТипНоменклатуры_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.ТипНоменклатуры)) КАК ТипНоменклатуры_UUID,
    ВЫБОР
        КОГДА Каталог.ТипНоменклатуры = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.ТипНоменклатуры)
    КОНЕЦ КАК ТипНоменклатуры_Тип
ИЗ
    Справочник.ВидыНоменклатуры КАК Каталог
"""
...
MAIN_QUERY_COLUMNS: List[str] = [
    ...
    "ТипНоменклатуры",
    "ТипНоменклатуры_Представление",
    "ТипНоменклатуры_UUID",
    "ТипНоменклатуры_Тип",
]

REFERENCE_COLUMNS: List[str] = ["ТипНоменклатуры"]
...
def load_nomenclature_types(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «ВИДЫ НОМЕНКЛАТУРЫ»")
    _log("=" * 80)
