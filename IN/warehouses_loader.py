# -*- coding: utf-8 -*-
"""
Загрузка справочника «Склады» из 1С в SQLite.
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    Каталог.ТипЦенРозничнойТорговли КАК ТипЦенРозничнойТорговли,
    ПРЕДСТАВЛЕНИЕ(Каталог.ТипЦенРозничнойТорговли) КАК ТипЦенРозничнойТорговли_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.ТипЦенРозничнойТорговли)) КАК ТипЦенРозничнойТорговли_UUID,
    ВЫБОР
        КОГДА Каталог.ТипЦенРозничнойТорговли = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.ТипЦенРозничнойТорговли)
    КОНЕЦ КАК ТипЦенРозничнойТорговли_Тип,
    ...
    Каталог.ВидСклада КАК ВидСклада,
    ПРЕДСТАВЛЕНИЕ(Каталог.ВидСклада) КАК ВидСклада_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.ВидСклада)) КАК ВидСклада_UUID,
    ВЫБОР
        КОГДА Каталог.ВидСклада = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.ВидСклада)
    КОНЕЦ КАК ВидСклада_Тип,
    Каталог.НомерСекции КАК НомерСекции,
    Каталог.РасчетРозничныхЦенПоТорговойНаценке КАК РасчетРозничныхЦенПоТорговойНаценке
ИЗ
    Справочник.Склады КАК Каталог
"""
...
MAIN_QUERY_COLUMNS: List[str] = [
    ...
    "ТипЦенРозничнойТорговли_Представление",
    "ТипЦенРозничнойТорговли_UUID",
    "ТипЦенРозничнойТорговли_Тип",
    "Подразделение",
    "Подразделение_Представление",
    "Подразделение_UUID",
    "Подразделение_Тип",
    "ВидСклада",
    "ВидСклада_Представление",
    "ВидСклада_UUID",
    "ВидСклада_Тип",
    "НомерСекции",
...
def load_warehouses(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «СКЛАДЫ»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Склады» не найдены.")
