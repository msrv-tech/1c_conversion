# -*- coding: utf-8 -*-
"""
Загрузка справочника «Назначения использования номенклатуры» из 1С в SQLite.
...
{limit_clause}    Каталог.Ссылка КАК Ссылка,
    ...
    Каталог.СпособПогашенияСтоимости КАК СпособПогашенияСтоимости,
    ПРЕДСТАВЛЕНИЕ(Каталог.СпособПогашенияСтоимости) КАК СпособПогашенияСтоимости_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.СпособПогашенияСтоимости)) КАК СпособПогашенияСтоимости_UUID,
    ВЫБОР
        КОГДА Каталог.СпособПогашенияСтоимости = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.СпособПогашенияСтоимости)
    КОНЕЦ КАК СпособПогашенияСтоимости_Тип,
    Каталог.СрокПолезногоИспользования КАК СрокПолезногоИспользования,
    ...
    Каталог.СпособОтраженияРасходов КАК СпособОтраженияРасходов,
    ПРЕДСТАВЛЕНИЕ(Каталог.СпособОтраженияРасходов) КАК СпособОтраженияРасходов_Представление,
    ПРЕДСТАВЛЕНИЕ(УНИКАЛЬНЫЙИДЕНТИФИКАТОР(Каталог.СпособОтраженияРасходов)) КАК СпособОтраженияРасходов_UUID,
    ВЫБОР
        КОГДА Каталог.СпособОтраженияРасходов = НЕОПРЕДЕЛЕНО
            ТОГДА ""
        ИНАЧЕ ТИПЗНАЧЕНИЯ(Каталог.СпособОтраженияРасходов)
    КОНЕЦ КАК СпособОтраженияРасходов_Тип
ИЗ
    Справочник.НазначенияИспользования КАК Каталог
"""
...
MAIN_QUERY_COLUMNS: List[str] = [
    ...
    "СпособПогашенияСтоимости_Представление",
    "СпособПогашенияСтоимости_UUID",
    "СпособПогашенияСтоимости_Тип",
    ...
    "СпособОтраженияРасходов_Представление",
    "СпособОтраженияРасходов_UUID",
    "СпособОтраженияРасходов_Тип",
]
...
def load_ppe_usage_purposes(
    ...
    _log("=" * 80)
    _log("ЗАГРУЗКА СПРАВОЧНИКА «НАЗНАЧЕНИЯ ИСПОЛЬЗОВАНИЯ НОМЕНКЛАТУРЫ»")
    _log("=" * 80)
    ...
    if not rows:
        _log("Предупреждение: записи справочника «Назначения использования номенклатуры» не найдены.")
